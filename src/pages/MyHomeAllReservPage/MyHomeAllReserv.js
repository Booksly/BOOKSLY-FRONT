...
function Testt({
    ...
    storeImage, storeName, location, category, isRegular,
    //예약한 날짜, 시간, 디자이너, 메뉴
    date, time, designer, menuList,
    // 예약목록 상태 'upcoming', 'cancelled', 'completed_with_review', 'completed_without_review' 
    status, 
    //평점, 리뷰내용
    reviewScore, reviewContent,
    onCancel, onDelete, onWriteReview, onDeleteReview}) 
{
            ...
            <div className="reservFrame">
                <div className="EmpBox">
                    <div className="EmpNameTime">
                        <div className={`EmpName ${status === 'cancelled' ? 'cancelLine' : ''}`}>{designer} -</div>
                        <div className="EmpTime">{time} 타임</div>
                        {status === 'cancelled' && <span className="cancelPartition">|</span>}
                        {status === 'cancelled' && <div className="cancelReserv">취소된 예약</div>}
                        {status === 'completed_with_review' && <span className="reviewPartition">|</span>}
                        {status === 'completed_with_review' && <div className="reviewScore">{reviewScore}</div>}
                        {status === 'completed_without_review' && <span className="reviewPartition">|</span>}
                        {status === 'completed_without_review' && <div className="reviewCreateButton" onClick={onWriteReview}>리뷰작성</div>}
                    </div>
                ...
                <div className="reservBoxLine"></div>
                {status === 'upcoming' && <div className="cancelButton" onClick={onCancel}>예약취소</div>}
                {status === 'cancelled' && <div className="cancelDelButton" onClick={onDelete}>취소내역삭제</div>}
                {status === 'completed_with_review' && (
                    <div className="reviewBox">
                        <div className="reviewLine"></div>
                        <div className="rvBoxIcon">
                            <div className="rvBox">Review</div>
                            <div className="rvDel" onClick={onDeleteReview}>리뷰삭제</div>
                        </div>
                ...
                {(status === 'completed_with_review' || status === 'completed_without_review') && (
                    <div className="DelButton" onClick={onDelete}>예약내역삭제</div>
                )}
            </div>
        </div>
    );
...
export default function MyHomeAllReserv() {
    ...
    const [reservations, setReservations] = useState([
        {
            ...
            menuList: '여성커트, 남성커트',
            status: 'upcoming',
            reviewContent: "마지막 타임 예약이 취소되서 북슬리에 올라온 거 빠르게 잡았어요. 원래는 이 시간에 미용실 가려면 이곳 저곳 전화해서 예약이 다찼는지 확인했어야 하는데, 비어있는 예약을 한번에 볼 수있어서 좋았습니다. 금용구 디자이너님 감사합니다~~",
        },
        {
            id: 901,
            // storeImage: Image1,
    ...
    const handleWriteReview = (id) => {
        setReservations(reservations.map(reservation =>
            reservation.id === id ? { ...reservation, status: 'completed_with_review' } : reservation
        ));
    };
    ...
    const handleDeleteReview = (id) => {
        setReservations(reservations.map(reservation =>
            reservation.id === id ? { ...reservation, status: 'completed_without_review' } : reservation
        ));
    };
